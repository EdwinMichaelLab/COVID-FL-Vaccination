...
function [tableConfirmed,tableDeaths,tableRecovered,time] = getDataCOVID_US(county)
...
dummy_filename = sprintf("%s.csv", county);
%test exists
if isfile(confirmed_filename)
    ...
    urlwrite(fullName,dummy_filename);
    tableConfirmed =readtable(dummy_filename, opts);
    delete(dummy_filename)
...
if isfile(deaths_filename)
    ...
    % File does not exist, then get data
    urlwrite(fullName,dummy_filename);
    tableDeaths =readtable(dummy_filename, opts);
    delete(dummy_filename)
