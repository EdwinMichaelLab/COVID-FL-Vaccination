...
bad_counties = ["Alachua", "Bay", "Jackson", "Monroe", "Okaloosa", "Santa Rosa", "Suwannee", "Walton", "Washington"];
...
if ~stateonly
    ...
    county_data = county_data(:, [8 13]);
end
if sum(isnan(table2array(county_data(:, 2)))) > 0 || stateonly
    disp("NaNs found. Using state-level data. (or stateonly is on...)");
    % Go with state data.
    state_data = state_data(strcmp(state_data.state_name, state), :);
    data = state_data(:, [6 11]); % daily visitation diff 
